...
export default function Softener() {
  ...
  //find the product record from my products array
  const softener = products.find(
    (product) => product.name?.toLowerCase() === "softener"
  );

  if (loading) return <p>Loading...</p>;
  if (!softener) return <p>Softener not found.</p>;

  //use that to filter records for only floor cleaner
  const softenerProductions = productions.filter(
    (p) => p.productId === softener?.id
  );

  const softenerSales = sales.filter((s) => s.productId === softener?.id);

  const softenerExpenses = expenses.filter(
    (e) => e.productId === softener?.id
  );


  //Compute Totals from the Fetched Data
  const totalProduced = softenerProductions.reduce((a, b) => a + b.quantity, 0);
  const totalSales = softenerSales.reduce((a, b) => a + b.quantity, 0);
  const totalRemaining = Math.max(totalProduced - totalSales, 0);
  const totalExpense = softenerExpenses
    ...
    toast((t) => (
      <div className="flex flex-col items-center gap-2">
        ...
        <div className="flex gap-3">
          <button
            onClick={() => {
              deleteExpense(softener.id, expenseId);
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div className="grid grid-cols-1 sm:grid-cols-2  items-center justify-between bg-pink-500 text-white px-6 py-4 rounded-xl shadow-md">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-green-100 text-gray-800 rounded-xl p-4 shadow relative">
          ...
          <button
            onClick={() => deleteLatestRecord("production", softener.id)}
        ...
        <div className="bg-blue-100 text-gray-800 rounded-xl p-4 shadow relative">
          ...
          {/* Undo button (top-right corner) */}
          <button
            onClick={() => deleteLatestRecord("sales", softener.id)}
      ...
      <div className="flex flex-wrap gap-4 font-semibold">
        ...
        <button
          ...
          className="flex items-center gap-2 bg-pink-600 hover:bg-pink-700 text-white px-4 py-2 rounded-lg shadow transition"
        >
      ...
      <div className="bg-white p-4 rounded-xl shadow-md">
        ...
        <table className="w-full text-sm text-left text-gray-700 border-collapse">
          ...
          <tbody>
            {softenerExpenses.length > 0 ? (
              softenerExpenses.map((expense) => (
                <tr
                  key={expense.id}
