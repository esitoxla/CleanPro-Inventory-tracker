...
export default function FloorCleaner() {
  ...
  const floorCleaner = products.find(
    (p) => p.name?.toLowerCase() === "floor cleaner"
  );

  if (loading) return <p>Loading...</p>;
  if (!floorCleaner) return <p>Floor Cleaner not found.</p>;

  //use that to filter records for only floor cleaner
  const floorProductions = productions.filter(
    (p) => p.productId === floorCleaner?.id
  );


  const floorSales = sales.filter((s) => s.productId === floorCleaner?.id);

  
  const floorExpenses = expenses.filter(
    (e) => e.productId === floorCleaner?.id
  );

  //Compute Totals from the Fetched Data
  const totalProduced = floorProductions.reduce((a, b) => a + b.quantity, 0);
  const totalSales = floorSales.reduce((a, b) => a + b.quantity, 0);
  const totalRemaining = Math.max(totalProduced - totalSales, 0);
  const totalExpense = floorExpenses
  ...
  // const totalProduced = produced || 0;
  // const totalSales = sales || 0;
  // const totalRemaining = Math.max(totalProduced - totalSales, 0); // avoid negatives
    ...
    toast((t) => (
      <div className="flex flex-col items-center gap-2">
        ...
        <div className="flex gap-3">
          <button
            onClick={() => {
              deleteExpense(floorCleaner.id, expenseId);
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 items-center justify-between bg-blue-500 text-white px-6 py-4 rounded-xl shadow-md">
        <h1 className="md:text-2xl text-[1.3rem] font-bold">
          Floor Cleaner Dashboard
        </h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-green-100 text-gray-800 rounded-xl p-4 shadow relative">
          ...
          <button
            onClick={() => deleteLatestRecord("production", floorCleaner.id)}
        ...
        <div className="bg-blue-100 text-gray-800 rounded-xl p-4 shadow relative">
          ...
          <button
            onClick={() => deleteLatestRecord("sales", floorCleaner.id)}
      ...
      <div className="bg-white p-4 rounded-xl shadow-md">
        ...
        <table className="w-full text-sm text-left text-gray-700 border-collapse">
          ...
          <tbody>
            {floorExpenses.length > 0 ? (
              floorExpenses.map((expense) => (
                <tr
                  key={expense.id}
